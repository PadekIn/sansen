<x-app-layout>
    ...
    <div class="section p-4">
        <form method="POST" action="{{ route('main.admin.update', $admin->id) }}">
            @csrf
            @method('PATCH')
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="email" class="form-control" disabled value="{{ old('email', $admin->email) }}">
                <input type="hidden" class="form-control" id="email" name="email"  value="{{ old('email', $admin->email) }}">
                @error('email')
                <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
